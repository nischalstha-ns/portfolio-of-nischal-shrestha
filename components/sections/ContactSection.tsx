            ...
            {PERSONAL_DETAILS.contacts
              ...
              <li key={contact.label} className="flex items-center">
                ...
                <div>
                  ...
                  {contact.link ? (
                    <a href={contact.link} className="text-sm text-text-light dark:text-text-dark hover:text-accent dark:hover:text-accent-dark break-all">
                      {contact.value}
                    </a>
                  ) : (
        ...
        <form onSubmit={handleSubmit} className="p-8 bg-secondary-light dark:bg-secondary-dark rounded-xl shadow-xl space-y-6">
          ...
          <div>
            ...
            <textarea name="message" id="message" rows={4} value={formData.message} onChange={handleChange} required
                      className="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-text-light dark:text-text-dark focus:ring-accent focus:border-accent transition-colors"></textarea>
          </div>
          <div>
            <button type="submit"
              ...
              Send Message <ion-icon name="send-outline" class="ml-2"></ion-icon>
            </button>
          </div>
          {isSubmitted && (
